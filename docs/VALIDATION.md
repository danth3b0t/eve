...
GitHub Actions run [34660867789](https://github.com/danth3b0t/eve/actions/runs/34660867789) passed the credential-free core suite, `go mod verify`, and `go vet -mod=readonly ./...` on `ubuntu-24.04`, `ubuntu-24.04-arm`, `macos-15`, and `macos-15-intel`. It rebuilt tag `v0.1.0-rc.11`, uploaded all four plain binaries and `SHA256SUMS.txt`, and the reviewed draft was published as a prerelease. The downloaded Linux/amd64 asset matched its checksum and responded with `eve 0.1.0-rc.11`; a clean `mise exec github:danth3b0t/eve@0.1.0-rc.11 -- eve version` probe passed. The second-generation alias workflow [34680797300](https://github.com/danth3b0t/eve/actions/runs/34680797300) verified the immutable assets, moved the floating Git `latest` tag, and published a documented full-release `latest` alias with identical assets; a clean `MISE_MINIMUM_RELEASE_AGE=0 mise exec github:danth3b0t/eve@latest -- eve version` probe also reported `0.1.0-rc.11`. Recorded checksums are in [RELEASE.md](RELEASE.md).
