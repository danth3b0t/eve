...
The binding `CONVEX_DEPLOYMENT=dev:<name>` plus the returned `CONVEX_DEPLOY_KEY=dev:<name>|<token>` was consumed successfully by CLI 1.45.0 through the unchanged root Bun/Turbo launch on Linux/amd64. The full live probe now passes: code push/selection, five-day deployment and key expiry metadata, inherited project development defaults, canonical URLs at the provider-default origin, cross-deployment key rejection, and exact cleanup. Regional origins and macOS remain untested. See [docs/M0.md](../../../docs/M0.md) for evidence and the runbook.
